...
macro_rules! count_args {
        ($($arg:ident),*) => { <[()]>::len(&[$($crate::boa::macros::replace_expr!($arg, ())),*]) };
    }
        ...
        ($js_ctx:ident : $fn:ident ($(
            ...
            boa_engine::object::FunctionObjectBuilder::new(
                ...
                boa_engine::native_function::NativeFunction::from_fn_ptr(
                    $crate::boa::macros::fn_body!($fn($($arg: $ty),* $(, [$ctx])?) -> $ret)
                ),
            )
            .length($crate::boa::macros::count_args!($($arg),*))
            .name(stringify!($fn))
            .build()
        };
    }
        ...
        ($js_ctx:ident : $fn:ident ($(
            ...
            boa_engine::object::FunctionObjectBuilder::new(
                ...
                boa_engine::native_function::NativeFunction::from_async_fn(
                    $crate::boa::macros::async_fn_body!($fn($($arg: $ty),* $(, [$ctx])?) -> $ret)
                ),
            )
            .length($crate::boa::macros::count_args!($($arg),*))
            .name(stringify!($fn))
...
macro_rules! module {
        ...
        $(
            ...
            {
            $(
                $(let $fn = $crate::boa::macros::fn_obj!($js_ctx : $fn($($tts)*) -> $ret);)?
                $(let $async_fn = $crate::boa::macros::async_fn_obj!($js_ctx : $async_fn($($async_tts)*) -> $async_ret);)?
            )*
            ...
            fn init(class: &mut boa_engine::class::ClassBuilder<'_>) -> JsResult<()> {
                $($(
                    class.method(
                        ...
                        $method_count,
                        boa_engine::NativeFunction::from_fn_ptr($crate::boa::macros::class_fn_wrap!($class, $method_fn)),
                    );
                )*)?
                Ok(())
            }
